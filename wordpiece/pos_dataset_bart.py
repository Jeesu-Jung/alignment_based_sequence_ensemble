...
class POS_Wordpiece_Dataset(torch.utils.data.Dataset):
    def __init__(self, data, tokenizer, max_len, label_vocab):
        ...
        for a_sent_char_and_label in data:
            ...
            _input = tokenizer(['<s> '+sentence], padding='max_length', max_length=max_len, return_tensors='pt')

            _label_ids = [label_vocab[x] for x in labels]
            # label_ids = _label_ids #for KOBART
            label_ids = [self.pad_id] + _label_ids
            if len(label_ids)>=max_len:
                # padded_label_ids = label_ids[:max_len]
            ...
            self.inputs.append(_input)
            self.outputs.append(padded_label_ids)
            _input_ids = _input['input_ids'].reshape(-1)
            _input_ids[len(tokens) + 1] = tokenizer.convert_tokens_to_ids('</s>')
            self.data.append(((_input_ids, _input['input_ids'].reshape(-1),
